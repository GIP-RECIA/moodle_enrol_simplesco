YUI.add('moodle-enrol_simplesco-cohortenrolment', function(Y) {
    ...
    Y.extend(USERENROLLER, Y.Base, {
        ...
        initializer : function(config) {
            ...
            this.set(UEP.BASE, create('<div class="'+CSS.PANEL+' '+CSS.HIDDEN+'"></div>')
                .append(create('<div class="'+CSS.WRAP+'"></div>')
                    .append(create('<div class="'+CSS.HEADER+' header"></div>')
                        ...
                        .append(create('<h2>'+M.str.enrol_simplesco.enrolcohorttitle+'</h2>')))
                    .append(create('<div class="'+CSS.CONTENT+'"></div>')
                        .append(create('<div class="'+CSS.SEARCHCONTROLS+'"></div>')
                            ...
                            .append(create('<div class="'+CSS.SEARCHOPTIONS+'"></div>')
                                .append(create('<div class="'+CSS.COLLAPSIBLEHEADING+' '+CSS.HIDDEN+'"><img alt="" />'+M.str.enrol.enrolmentoptions+'</div>'))
                            ...
                            .append(create('<div class="'+CSS.SEARCHOPTIONS2+'"></div>')
                                ...
                                .append(create('<div class="'+CSS.COLLAPSIBLEAREA+' '+CSS.HIDDEN+'"></div>')
                                    ...
                                    .append(create('<div class="'+CSS.SEARCH+'"><label for="enrolusersearch" class="accesshide">'+M.str.enrol.usersearch+'</label></div>')
                                        .append(create('<label for="enrolusersearch">'+M.str.enrol_simplesco.fitre6namecohort+' : </label><input type="text" id="enrolusersearch" value="" />'))
                                        .append(create('<br><input type="button" id="searchbtn" class="'+CSS.SEARCHBTN+'" value="'+M.str.enrol.usersearch+'" />'))
                                    )
                                )
                            )
                        )
                    ...
                    .append(create('<div class="'+CSS.FOOTER+'"></div>')
                        .append(create('<div class="'+CSS.CLOSEBTN+'"></div>')
                            .append(create('<input type="button" value="'+M.str.enrol_simplesco.btnclosecohort+'" />'))
                        )
                    )
                )
            );
            ...
            Y.all('.user-choiseenroller-panel .btn-cohorte button').each(function(node){
        ...
        search : function(e, append) {
            ...
            Y.io(M.cfg.wwwroot+this.get(UEP.AJAXURL), {
                ...
                on : {
                    ...
                    complete: this.processCohortsSearchResults,
        ...
        processCohortsSearchResults : function(tid, outcome, args) {
            ...
            var $option ='';
            for (var g in result.response.group) {
            	$option = $option+'<option value="'+g+'">'+result.response.group[g]+'</option>';
            }
            ...
            for (var i in result.response.cohorts) {
                ...
                cohorts.append(create('<div class="'+CSS.COHORT+' clearfix" rel="'+cohort.id+'"></div>')
                    ...
                    .append(create('<div class="'+CSS.OPTIONS+'"></div>')
                    	.append(create('<select id="sel'+cohort.id+'" style="font-size: 1.1em; margin: 0px; padding: 0 0px; height: 22px; line-height: 22px;">'+$option+'</select>'))	
                        .append(create('<input type="button" class="'+CSS.ENROL+'" value="'+M.str.enrol.enrol+'" />')) 
                     ) 
                );
            }
            ...
            if (!args.append) {
                ...
                var content = create('<div class="'+CSS.SEARCHRESULTS+'"></div>')
                    ...
                    .append(create('<div class="'+CSS.COHORT+' clearfix"><div class="'+CSS.COUNT2+'">N°</div><div class="'+CSS.DETAILS+'">'+M.str.enrol_simplesco.cohort+'</div><div class="'+CSS.OPTIONS+'">'+M.str.enrol_cohort.addgroup+'</div></div>'))
                    .append(cohorts);
        ...
        enrolUser : function(e, args) {
            ...
            var nomsel = "sel"+entity.getAttribute("rel");
            var sel = document.getElementById(nomsel);
            var params = [];
            params['id'] = this.get(UEP.COURSEID);
            params['cohortid'] = entity.getAttribute("rel");
            params['groupeid'] = sel.options[sel.selectedIndex].value;
            params['enrolid'] = args.enrolid;
            params['sesskey'] = M.cfg.sesskey;
            params['action'] = 'enrolcohort';
    ...
    M.enrol_simplesco.cohortenrolment = {
        init : function(cfg) {
