...
d+='<div class="form-group"><label for="enrol_simplesco_liste5">'+a[16]+' :</label><select id="enrol_simplesco_liste5" name="code5" style="max-width: 300px; width:100%;"></select></div>';d=d+'</div><div id="enrol_simplesco_input form-group">'+('<label for="enrolusersearch">'+a[17]+' :</label> <input type="text" id="enrolusersearch" value="" class="form-control" style="max-width: 300px; width:100%;"/>');d=d+"</div></div></fieldset>"+('<div class="text-center"><button class="btn btn-primary" id="search"> '+
a[18]+"</button></div>");k.setBody(d+'<div id="results"></div><div id="loading" style="display:none;">CHARGEMENT</div></div>');k.setFooter('<button class="btn btn-primary" data-action="hide">'+a[1]+"</button>");k.getRoot().on(p.hidden,function(){c.require_refresh&&location.reload()});this.bindEvents();this.displayLists();this.populateRoles();this.updateStartDateList();this.updateDurationsList()}.bind(this)).fail(h.exception)};e.prototype.bindEvents=function(){var c=this;f("body").on("click","button#enrol_simplesco_adduser",
