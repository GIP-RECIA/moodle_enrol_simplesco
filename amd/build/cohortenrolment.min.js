...
define("core/templates jquery core/str core/config core/notification core/modal_factory core/modal_events core/fragment".split(" "),function(a,d,g,h,f,k,l,m){var e=function(c){this.course_id=c.course_id;this.instance=c.instance;this.start_dates=c.start_dates;this.init()};e.prototype.modal=null;e.prototype.page=0;e.prototype.perpage=25;e.prototype.course_id=0;e.prototype.enrol_count=0;e.prototype.assignales_roles=[];e.prototype.instance=[];e.prototype.start_dates=[];e.prototype.groups=[];e.prototype.require_refresh=
!1;e.prototype.init=function(){var c=d("button#enrol_simplesco_addcohort");d.when(g.get_strings([{component:"enrol_simplesco",key:"enrolcohorttitle"},{component:"enrol_simplesco",key:"btnclosecohort"},{component:"role",key:"assignroles"},{component:"enrol",key:"none"},{component:"enrol",key:"enrolmentoptions"},{component:"moodle",key:"startingfrom"},{component:"enrol",key:"enrolperiod"},{component:"enrol",key:"unlimitedduration"},{component:"enrol_simplesco",key:"searchoption"},{component:"enrol",
key:"usersearch"},{component:"enrol_simplesco",key:"noresultliste"},{component:"enrol_simplesco",key:"allresultliste"},{component:"enrol_simplesco",key:"ajaxxcohortfound"},{component:"enrol",key:"ajaxnext25"},{component:"enrol",key:"enrol"},{component:"enrol_simplesco",key:"cohort"},{component:"enrol_cohort",key:"addgroup"}]),k.create({type:k.types.DEFAULT,large:!0},c)).then(function(c,a){this.strings=c;this.modal=a;a.setTitle(c[0]);var d='<div class="form-inline mform simplesco"><div class="form-group">'+
('<label for="enrol_simplesco_assignable_roles">'+c[2]+" : </label>");d+='<select id="enrol_simplesco_assignable_roles"><option value="">'+c[3]+"</option></select>";d=d+'</div><fieldset class="collapsible collapsed" id="enrolment_options">'+('<legend class="ftoggler"><a href="#" class="fheader" role="button" aria-controls="enrolment_options" aria-expanded="false">'+c[4]+"</a></legend>");d=d+'<div class="fcontainer">'+('<div class="startdate form-group"><label for="enrol_simplesco_assignable_startdate">'+
c[5]+':</label> <select id="enrol_simplesco_assignable_startdate"></select></div>');d+='<div class="duration form-group"><label for="enrol_simplesco_assignable_duration">'+c[6]+' :</label> <select id="enrol_simplesco_assignable_duration"><option value="0" selected="selected">'+c[7]+"</option></select></div>";d=d+'</div></fieldset><fieldset class="collapsible collapsed" id="enrolment_options">'+('<legend class="ftoggler"><a href="#" class="fheader" role="button" aria-controls="enrolment_options" aria-expanded="false">'+
c[8]+"</a></legend>");d=d+'<div class="fcontainer">'+('<label for="enrolusersearch">'+c[9]+' :</label> <input type="text" id="enrolusersearch" value="" class="form-control"/>');d=d+"</div></fieldset>"+('<div class="text-center"><button class="btn btn-primary" id="search"> '+c[9]+"</button></div>");a.setBody(d+'<div id="results"></div><div id="loading" style="display:none;">CHARGEMENT</div></div>');a.setFooter('<button class="btn btn-primary" data-action="hide">'+c[1]+"</button>");a.getRoot().on(l.hidden,
function(){b.require_refresh&&location.reload()});this.bindEvents();this.populateRoles();this.updateStartDateList();this.updateDurationsList()}.bind(this)).fail(f.exception);var b=this;d("body").on("click","button#enrol_simplesco_addcohort",function(){b.modal.show()})};e.prototype.bindEvents=function(){var c=this;d("body").on("click","button#enrol_simplesco_addcohort",function(){c.modal.show()});this.modal.getBody().find(".collapsible .ftoggler a").on("click",function(b){b.preventDefault();d(this).closest(".collapsible").toggleClass("collapsed");
...
c.id+'">';a=a+('<div class="count">'+(b+1)+"</div>")+'<div class="details">'+('<div class="fullname">'+c.name+"</div>");a+="</div>";a+='<div class="options">';a+='<select name="cohort_group" rel="'+c.id+'">';for(var d in this.groups)a+='<option value="'+d+'">'+this.groups[d]+"</option>";a+="</select>";a+='<button class="btn btn-success" data-toggle="enrol" value="'+c.id+'">'+this.strings[14]+"</button>";a+="</div>";return a+="</div>"};e.prototype.search=function(a){var b=this;a?this.page++:this.page=
...
c.response.totalusers&&b.modal.getBody().find("div#results").append('<div class="text-center"><button class="btn btn-secondary" data-toggle="load-more">'+b.strings[13]+"</button></div>")},error:function(a,c,b){f.exception(b)},beforeSend:function(){b.startLoading()}}).always(function(){b.stopLoading()})};e.prototype.startLoading=function(){this.modal.getBody().find("#loading").show()};e.prototype.stopLoading=function(){this.modal.getBody().find("#loading").hide()};e.prototype.updateRolesList=function(){var a=
this.modal.getBody().find("select#enrol_simplesco_assignable_roles"),b;for(b in this.assignales_roles)d(a).append('<option value="'+this.assignales_roles[b].id+'">'+this.assignales_roles[b].name+"</option>")};e.prototype.updateDurationsList=function(){var a=this.modal.getBody().find("select#enrol_simplesco_assignable_duration");d.when(g.get_string("durationdays","enrol","{a}")).then(function(b){for(var c=1;365>=c;c++)d(a).append('<option value="'+c+'">'+b.replace("{a}",c)+"</option>")})};e.prototype.updateStartDateList=
function(){var a=this.modal.getBody().find("select#enrol_simplesco_assignable_startdate"),b;for(b in this.start_dates)d(a).append('<option value="'+b+'">'+this.start_dates[b]+"</option>")};return{init:function(a){new e(a)}}});
