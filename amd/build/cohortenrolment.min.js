...
this.modal.getBody().find("select#enrol_simplesco_assignable_roles"),b;for(b in this.assignales_roles){var e=this.role_default==this.assignales_roles[b].id?'selected="selected"':"";d(a).append('<option value="'+this.assignales_roles[b].id+'" '+e+">"+this.assignales_roles[b].name+"</option>")}};e.prototype.updateDurationsList=function(){var a=this.modal.getBody().find("select#enrol_simplesco_assignable_duration");d.when(g.get_string("durationdays","enrol","{a}")).then(function(b){for(var c=1;365>=
c;c++)d(a).append('<option value="'+c+'">'+b.replace("{a}",c)+"</option>")})};e.prototype.updateStartDateList=function(){var a=this.modal.getBody().find("select#enrol_simplesco_assignable_startdate"),b;for(b in this.start_dates)d(a).append('<option value="'+b+'">'+this.start_dates[b]+"</option>")};return{init:function(a){new e(a)}}});
